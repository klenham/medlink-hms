...
import { createDraftClaimFromConsultation, createDraftClaimFromPrescription, validateClaimItems, runClaimOptimization, buildClaimsCsv, CLAIM_STATUSES } from '../lib/claims.js';
...
router.post('/:id/ai-review', authenticateToken, async (req, res) => {
  try {
    const result = await runClaimOptimization(req.params.id);
    res.json(result);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
